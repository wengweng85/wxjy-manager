			...
			<div class="ibox float-e-margins">
				<div class="ibox-title">
					...
					<div class="ibox-tools">
                        ...
                        <a onclick="open_code_type_sort_mg()" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i>&nbsp;代码类型分类管理</a>
                        <rc:hidden property="q_code_type" value="${codetype.code_type}"/>
...
<div id="rDetailMenu">
    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
	    ...
	    <li id="m_detail_sub_add" onclick="addDetailSubTreeNode();"><a  tabindex="-1" href="#"><i class="fa fa-chevron-down"></i>新增下级代码值</a></li>
...
var sys_code_type_setting = {
	  ...
	  async: {
		 enable: true,
		 url: "<c:url value='/sys/codetype/codetype_treedata'/>",
		 autoParam:["id"],
...
var sys_code_type_setting_detail = {
	...
	async: {
		enable: true,
		url: "<c:url value='/sys/codetype/codevalue_treedata'/>",
		autoParam:["id"]
	}
};
...
function add_code_type(){
	layer.open({
	  ...
	  shade: 0.8,
	  area: ['50%', '60%'],
	  content: "<c:url value='/sys/codetype/toCodeTypeAdd'/>" //iframe的url
	});
}
...
function edit_code_type(id){
	layer.open({
	  ...
	  shade: 0.8,
	  area: ['50%', '60%'],
	  content: "<c:url value='/sys/codetype/toCodeTypeEdit'/>/"+id //iframe的url
	});
}
	...
	if (nodes && nodes.length>0) {
		if (nodes[0].children && nodes[0].children.length > 0) {
			...
			layer.confirm('确定删除要此代码,此功能原则上只有开发人员才可以操作,删除后系统相关业务代码需要同步更新', function(index){
	   			var url= "<c:url value='/sys/codetype/deleteCodeType/'/>"+current_selectedNodes.id;
...
function add_detail_code_type(){
	if($('#code_type').val()){
		layer.open({
			  ...
			  shade: 0.8,
			  area: ['50%', '60%'],
			  content: "<c:url value='/sys/codetype/toCodeTypeDetailAddFromRoot'/>/"+$('#code_type').val() //iframe的url
			});
	}else{
		layer.msg('请先选择代码类型');
	}
...
function add_detail_sub_code_type(code_seq){
	layer.open({
		  ...
		  shade: 0.8,
		  area: ['50%', '60%'],
		  content: "<c:url value='/sys/codetype/toCodeTypeDetailAddFromNode'/>/"+code_seq //iframe的url
	});
}
/**
 * 新增明细回调函数
...
function edit_detail_code_type(id){
	layer.open({
	  ...
	  shade: 0.8,
	  area: ['50%', '60%'],
	  content: "<c:url value='/sys/codetype/toCodeTypeDetailEdit'/>/"+id //iframe的url
	});
}
	...
	if (nodes && nodes.length>0) {
		...
		if (nodes[0].children && nodes[0].children.length > 0) {
			...
			layer.confirm('确定删除要此代码值,此功能原则上只有开发人员才可以操作,删除后系统相关业务程序代码需要同步更新', function(index){
	   			var url= "<c:url value='/sys/codetype/deleteCodeValue/'/>"+current_detail_selectedNodes.code_seq;
	   			rc.ajax(url, null,function (response) {
	   				if(response.success){
...
function open_code_type_query(){
	layer.open({
	  ...
	  offset: ['100px', '50px'],
	  area: ['45%', '40%'],
	  content:"<c:url value='/sys/codetype/toCodeTypeQuery'/>"     //iframe的url
	});
}
 ...
 * 打开代码类别分类管理
 */
function open_code_type_sort_mg(){
	layer.open({
	    type: 2,
	    title: '代码类别分类管理',
	    shadeClose: false,
	    ...
	    shade: 0.8,
	    area: ['70%', '80%'],
	    content:"<c:url value='/sys/codetype/codetypesort/index'/>"     //iframe的url
	});
}
...
function open_code_value_query(){
	if($('#code_type').val()){
		layer.open({
			  ...
			  shade: 0.8,
			  area: ['45%', '40%'],
			  content:"<c:url value='/sys/codetype/toCodeValueQuery'/>"     //iframe的url
			});
	}else{
		layer.msg('请先选择代码类型');
	}
