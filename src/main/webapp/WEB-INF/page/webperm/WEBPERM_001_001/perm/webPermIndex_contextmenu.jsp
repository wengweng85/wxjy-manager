...
<div class="ibox ">
	<div class="ibox-title">
		...
		<div class="ibox-tools">
		    <button id="tree_refresh" onclick="tree_refresh()" class="btn btn-primary btn-xs"><i class="fa fa-refresh"></i>刷新</button>
		    <button id="tree_expand"  class="btn btn-primary btn-xs"><i class="fa fa-plus"></i>展开</button>
                     <a onclick="s_perm_bat_delete()" class="btn btn-danger btn-xs"><i class="fa fa-remove"></i>批量删除</a>
                 </div>
	</div>
	...
	<div class="ibox ">
		...
		<form action="<c:url value='/web/perm/saveorupdate'/>"  class="form-horizontal" method="post" id="myform">
           ...
           <div class="form-group">
                <rc:textedit property="parentid"  readonly="true" cols="2,10" label="上级权限编码 " />
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="parentname" cols="2,10" label="上级权限名称 " readonly="true"/>
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="code"  required="true"  cols="2,10" label="权限编码 " validate="{required:true,messages:{required:'请输入权限编码'}}"/>
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="name"  required="true"  cols="2,10" label="权限名称" validate="{required:true,messages:{required:'请输入权限名称'}}"/>
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
               <rc:select codetype="PERMTYPE" property="type"  label="权限类型" required="true"  cols="2,10" validate="{required:true,messages:{required:'请选择权限类型'}}"/> 
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="url"  required="true"  value="#" cols="2,10" label="权限地址"  validate="{required:true,messages:{required:'权限地址不能为空'}}" />
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="sortnum"  required="true"  type="number" cols="2,10" datamask="99" label="排序号" validate="{required:true,messages:{required:'排序号不能为空'}}"/>
            </div>
            <div class="hr-line-dashed"></div>
			<div class="form-group">
			<rc:select  property="systypecode" label="所属应用类型" required="true"  cols="2,10"  codetype="SYSTYPECODE"  validate="{required:true,messages:{required:'请输入所属应用类型'}}" />
			</div>
            <div class="hr-line-dashed"></div>
            <div class="form-group">
                 <rc:checkboxswitch label="是否有效" property="enabled" value="1"   cols="2,10" />
           </div>
           <div class="hr-line-dashed"></div>
            <div class="form-group">
               <rc:checkboxswitch label="是否空白窗口打开" property="isblanktarget"  cols="2,10" />
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="describe" cols="2,10" label="权限描述" />
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="iconcss" cols="2,10" label="图标样式" readonly="true" onclick="open_font_css_iframe()" />
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group" style="text-align: right;" >
           <div class="col-sm-10 col-xs-10">
             <a class="btn btn-primary "  onclick="s_perm_savePermData()"><i class="fa fa-save"></i>&nbsp;保存</a>
          </div>
       </div>
      </form>
		</div>
...
<div id="rDetailMenu">
   <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
	   ...
	   -->
	   <li id="m_detail_sub_add" onclick="addDetailSubTreeNode();"><a  tabindex="-1" href="#"><i class="fa fa-chevron-down"></i>新增下级权限</a></li>
...
function s_perm_bat_delete(){
    ...
    if(nodes.length>0){
	   	 ...
	   	 layer.confirm('确定删除先中的'+nodes.length+'条权限,此功能原则上只有开发人员才可以操作,删除后系统相关业务程序代码需要同步更新', function(index){
  			var url= "<c:url value='/web/perm/batdeletePermData/'/>";
  			rc.ajax(url, {selectnodes:selectnodes},function (response) {
  				if(response.success){
  					tree_refresh();
  				}else{
  					alert(response.message);
...
function zTreeOnDrop(event, treeId, treeNodes, targetNode, moveType) {
     //for(i=0;i<treeNodes.length;i++){
    	 ...
    	 if(targetNode){
    		 var url= "<c:url value='/web/perm/moveNode/'/>"+treeNodes[0].id+"_"+targetNode.id;
...
function sys_right_menu_perm_addnewperm(id,name,systypecode){
	rc.clean();
	$('#parentid').val(id);
	$('#parentname').val(name);
	$('#url').val('#');
	$('#systypecode').selectpicker('val',systypecode);
...
function addDetailSubTreeNode() {
	...
	current_detail_selectedNodes=zDetailTree.getSelectedNodes()[0];
	sys_right_menu_perm_addnewperm(current_detail_selectedNodes.id,current_detail_selectedNodes.name,current_detail_selectedNodes.systypecode);
}
...
function editDetailTreeNode() {
	...
	if (zDetailTree.getSelectedNodes()[0]) {
		...
		current_detail_selectedNodes=zDetailTree.getSelectedNodes()[0];
		//调用编辑弹出页面
		rc.ajaxQuery("<c:url value='/web/perm/getPermData/'/>"+current_detail_selectedNodes.id);
		$('#optype').val('2');//1 新增结点  2更新结点
	...
	if (nodes && nodes.length>0) {
		...
		if (nodes[0].children && nodes[0].children.length > 0) {
			...
			layer.confirm('确定删除要此权限,此功能原则上只有开发人员才可以操作,删除后系统相关业务程序代码需要同步更新', function(index){
	   			var url= "<c:url value='/web/perm/deletePermDataById/'/>"+current_detail_selectedNodes.id;
	   			rc.ajax(url, null,function (response) {
	   				if(response.success){
...
function s_perm_deleteperm(){
	...
	if(id){
		layer.confirm('确定删除要此权限吗？', function(index){
			var url= "<c:url value='/web/perm/deletePermDataById/'/>"+id;
			rc.ajax(url, null,function (response) {
				if(response.success){
					s_perm_treeinit();
					rc.clean();
