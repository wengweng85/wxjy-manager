...
var sys_code_type_setting = {
	  ...
	  async: {
		 enable: true,
		 url: "<c:url value='/web/codetype/codetype_treedata'/>",
		 autoParam:["id"],
...
var sys_code_type_setting_detail = {
	...
	async: {
		enable: true,
		url: "<c:url value='/web/codetype/codevalue_treedata'/>",
		autoParam:["id"]
	}
};
...
function add_code_type(){
	layer.open({
	  ...
	  shade: 0.8,
	  area: ['50%', '60%'],
	  content: "<c:url value='/web/codetype/toCodeTypeAdd'/>" //iframe的url
	});
}
...
function edit_code_type(id){
	layer.open({
	  ...
	  shade: 0.8,
	  area: ['50%', '60%'],
	  content: "<c:url value='/web/codetype/toCodeTypeEdit'/>/"+id //iframe的url
	});
}
	...
	if (nodes && nodes.length>0) {
		if (nodes[0].children && nodes[0].children.length > 0) {
			...
			layer.confirm('确定删除要此代码,此功能原则上只有开发人员才可以操作,删除后系统相关业务代码需要同步更新', function(index){
	   			var url= "<c:url value='/web/codetype/deleteCodeType/'/>"+current_selectedNodes.id;
...
function add_detail_code_type(){
	if($('#code_type').val()){
		layer.open({
			  ...
			  shade: 0.8,
			  area: ['50%', '60%'],
			  content: "<c:url value='/web/codetype/toCodeTypeDetailAddFromRoot'/>/"+$('#code_type').val() //iframe的url
			});
	}else{
		layer.msg('请先选择代码类型');
	}
...
function add_detail_sub_code_type(code_seq){
	layer.open({
		  ...
		  shade: 0.8,
		  area: ['50%', '60%'],
		  content: "<c:url value='/web/codetype/toCodeTypeDetailAddFromNode'/>/"+code_seq //iframe的url
	});
}
/**
 * 新增明细回调函数
...
function edit_detail_code_type(id){
	layer.open({
	  ...
	  shade: 0.8,
	  area: ['50%', '60%'],
	  content: "<c:url value='/web/codetype/toCodeTypeDetailEdit'/>/"+id //iframe的url
	});
}
	...
	if (nodes && nodes.length>0) {
		...
		if (nodes[0].children && nodes[0].children.length > 0) {
			...
			layer.confirm('确定删除要此代码值,此功能原则上只有开发人员才可以操作,删除后系统相关业务程序代码需要同步更新', function(index){
	   			var url= "<c:url value='/web/codetype/deleteCodeValue/'/>"+current_detail_selectedNodes.code_seq;
	   			rc.ajax(url, null,function (response) {
	   				if(response.success){
...
function open_code_type_query(){
	layer.open({
	  ...
	  offset: ['100px', '50px'],
	  area: ['45%', '40%'],
	  content:"<c:url value='/web/codetype/toCodeTypeQuery'/>"     //iframe的url
	});
}
...
function open_code_value_query(){
	if($('#code_type').val()){
		layer.open({
			  ...
			  shade: 0.8,
			  area: ['45%', '40%'],
			  content:"<c:url value='/web/codetype/toCodeValueQuery'/>"     //iframe的url
			});
	}else{
		layer.msg('请先选择代码类型');
	}
